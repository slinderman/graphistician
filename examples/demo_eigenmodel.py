...
def demo(seed=None):
    ...
    true_model = Eigenmodel(N=N, D=D, p=p, r=3.0)
    ...
    test_model = Eigenmodel(N=N, D=D, p=p)
    # Override the latent feature metric
    test_model.lmbda = np.ones((2,))
    ...
    for smpl in xrange(N_samples):
        ...
        ax_test.cla()
        test_model.plot(A, ax=ax_test, color='b', F_true=true_model.F)
        plt.pause(0.001)
